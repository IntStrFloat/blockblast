...
Принципы: ничего не анимируем через JS-state; только `useAnimatedStyle` + shared values; стаггеры — `withDelay`. Częстицы и конфетти — фиксированные пулы компонентов (без mount/unmount в горячем пути).
